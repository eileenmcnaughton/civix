...
<?php if (!empty($credential2)) { echo
  "        \$this->gateway->set" . $credential2_camel . "('really_secure');\n";
    ...
    public function testAuthorize()
    {
        ...
        $this->assertInstanceOf('Omnipay\<?php echo $processorName ?>\Message\OffsiteAuthorizeResponse', $response);
        $this->assertFalse($response->isSuccessful());
        $this->assertTrue($response->isRedirect());
        $this->assertNotEmpty($response->getRedirectUrl());
    ...
    public function testCompletePurchase()
    {
        ...
        $this->assertInstanceOf('Omnipay\<?php echo $processorName ?>\Message\OffsiteCompletePurchaseRequest', $request);
